...
//        Assertions.assertEquals(expected, result, 1);
    ...
    void testAssertNotEqualsWithInstaces() {
    ...
    void testAssertAll(){
        ...
        Assertions.assertAll("그룹화된 테스트의 이름(테스트 실패 시 보여짐", () ->
                Assertions.assertEquals(firstName, person.getFirstname(), () -> "firstName이 잘못됨")
                        , () -> Assertions.assertEquals(lastName, person
                                .getLastname(), () -> "lastName이 잘못됨"));
    }
    ...
    void testAssertThrows(){
        ...
        Exception exception = Assertions.assertThrows(Exception.class, () -> validator.checkDividableNumber(firstNum, secondNum));

        Assertions.assertAll(() -> Assertions.assertInstanceOf(IllegalArgumentException.class, exception, () -> "예외 타입이 일치하지 않음"),
                () -> Assertions.assertEquals(expectedErrorMessage, exception.getMessage(), () -> "예외메세지가 일치하지 않음"));
